...
func TestStudioGenerate_streamsProgressThenResult(t *testing.T) {
	fake := fakeStudio{result: studio.GenerateResult{
		...
		Bands: []string{"Ashen Verdict"}, Titles: []string{"Sleep Now", "Iron Lullaby"}, Albums: []string{"Nightfall Sessions"},
	}}
	...
	if !strings.Contains(body, "Ashen Verdict") || !strings.Contains(body, "Sleep Now") || !strings.Contains(body, "Nightfall Sessions") {
		t.Fatalf("result event missing band/title/album ideas: %s", body)
	}
}

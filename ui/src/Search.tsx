                ...
                <section>
                  ...
                  >
                    ...
                    <span>
                      ...
                      <div style={{ ...t.label, textTransform: "capitalize" }}>
                        {res.top.type}
                      </div>
                    </span>
                  </button>
            ...
            <section>
              ...
              {res.songs.map((s, i) => {
                ...
                return (
                  <button
                    key={s.id}
                    onClick={() => onPlay(s, res.songs.slice(i + 1))}
                    style={rowBtn}
                  >
                    <SongCover
                      song={s}
              ...
              {res.artists.map((a) => (
                ...
                >
                  ...
                  <span>
                    ...
                    <span style={{ display: "block", ...t.label }}>
                      {a.songCount} {a.songCount === 1 ? "song" : "songs"}
                    </span>
                  </span>
                </button>
              ))}
                ...
                >
                  <span
                    style={{
                      ...
                      height: 40,
                      borderRadius: 6,
                      background: g.accentColor || "var(--color-active)",
                    }}
                  />
...
function topLabel(res: SearchResults): string {
  ...
  if (type === "artist")
    return res.artists.find((x) => x.id === id)?.name ?? "";
  if (type === "genre") {
    const n = res.genres.find((x) => x.id === id)?.name;
    return n ? genreLabel(n) : "";
  }
...
function topCoverArtId(res: SearchResults): string {
  ...
  const { type, id } = res.top;
  if (type === "song")
    return res.songs.find((x) => x.id === id)?.coverArtId ?? "";
  if (type === "playlist")
    return res.playlists.find((x) => x.id === id)?.coverArtId ?? "";
  return "";
}

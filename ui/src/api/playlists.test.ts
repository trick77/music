...
describe("playlist reads", () => {
  ...
  it("getPlaylist surfaces the status in the error", async () => {
    mockFetch({}, false, 404);
    await expect(getPlaylist("nope")).rejects.toThrow(
      "failed to load playlist (404)",
    );
  });
});
...
describe("playlist writes", () => {
  ...
  it("createPlaylist throws with the status", async () => {
    mockFetch({}, false, 400);
    await expect(createPlaylist("x", "")).rejects.toThrow(
      "create failed (400)",
    );
  });
  ...
  it("updatePlaylist throws with the status", async () => {
    mockFetch({}, false, 409);
    await expect(updatePlaylist("p1", "a", "b")).rejects.toThrow(
      "save failed (409)",
    );
  });
  ...
  it("updatePlaylistDescription throws with the status", async () => {
    mockFetch({}, false, 500);
    await expect(updatePlaylistDescription("p1", "x")).rejects.toThrow(
      "save failed (500)",
    );
  });
});
...
describe("playlist membership", () => {
  ...
  it("addSongToPlaylist throws with the status", async () => {
    mockFetch({}, false, 404);
    await expect(addSongToPlaylist("p1", "s1")).rejects.toThrow(
      "add failed (404)",
    );
  });
  ...
  it("removeSongFromPlaylist DELETEs the nested song path", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/playlists/p1/songs/s1", {
      method: "DELETE",
    });
  ...
  it("removeSongFromPlaylist throws with the status", async () => {
    mockFetch({}, false, 404);
    await expect(removeSongFromPlaylist("p1", "s1")).rejects.toThrow(
      "remove failed (404)",
    );
  });
  ...
  it("reorderPlaylist PUTs the full id order", async () => {
    ...
    expect(f).toHaveBeenCalledWith(
      "/api/playlists/p1/reorder",
      expect.objectContaining({ method: "PUT", headers: JSON_HEADERS }),
    );
  ...
  it("reorderPlaylist throws with the status", async () => {
    mockFetch({}, false, 400);
    await expect(reorderPlaylist("p1", [])).rejects.toThrow(
      "reorder failed (400)",
    );
  });
});
...
describe("playlist publishing", () => {
  it("publishes via the publish path", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/playlists/p1/publish", {
      method: "POST",
    });
  ...
  it("unpublishes via the unpublish path", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/playlists/p1/unpublish", {
      method: "POST",
    });
  });
  ...
  it("throws with the status", async () => {
    mockFetch({}, false, 401);
    await expect(setPlaylistPublished("p1", true)).rejects.toThrow(
      "playlist publish toggle failed (401)",
    );
  });
});
...
describe("playlist cover art", () => {
  ...
  it("uploadPlaylistCover throws with the status", async () => {
    mockFetch({}, false, 413);
    await expect(
      uploadPlaylistCover("p1", new File(["x"], "a.png")),
    ).rejects.toThrow("cover upload failed (413)");
  });
  ...
  it("applyPlaylistCover throws with the status", async () => {
    mockFetch({}, false, 404);
    await expect(applyPlaylistCover("p1", "studio-7")).rejects.toThrow(
...
describe("playlist AI helpers", () => {
  it("suggestPlaylistPrompt POSTs with no body", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/playlists/p1/suggest-prompt", {
      method: "POST",
    });
  ...
  it("suggestPlaylistPrompt throws with the status", async () => {
    mockFetch({}, false, 503);
    await expect(suggestPlaylistPrompt("p1")).rejects.toThrow(
      "suggest failed (503)",
    );
  });
  ...
  it("refinePlaylistPrompt sends the current prompt and the instruction", async () => {
    ...
    const res = await refinePlaylistPrompt(
      "p1",
      "neon skyline",
      "make it dusk",
    );
    expect(f).toHaveBeenCalledWith(
      "/api/playlists/p1/refine-prompt",
      expect.objectContaining({ method: "POST", headers: JSON_HEADERS }),
    );
    expect(sentJson(f)).toEqual({
      current: "neon skyline",
      instruction: "make it dusk",
    });
  ...
  it("refinePlaylistPrompt throws with the status", async () => {
    mockFetch({}, false, 500);
    await expect(refinePlaylistPrompt("p1", "a", "b")).rejects.toThrow(
      "refine failed (500)",
    );
  });
  ...
  it("suggestPlaylistDescriptions returns the three variants", async () => {
    const f = mockFetch({
      punchy: "Loud.",
      evocative: "Rain on glass.",
      factual: "12 tracks.",
    });
    const res = await suggestPlaylistDescriptions("p1");
    expect(f).toHaveBeenCalledWith("/api/playlists/p1/suggest-description", {
      method: "POST",
    });
    expect(res).toEqual({
      punchy: "Loud.",
  ...
  it("suggestPlaylistDescriptions throws with the status", async () => {
    mockFetch({}, false, 503);
    await expect(suggestPlaylistDescriptions("p1")).rejects.toThrow(
      "suggest failed (503)",
    );
  });
});

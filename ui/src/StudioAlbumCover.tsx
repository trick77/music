...
export function AlbumCoverMode({ chatEnabled, imageModels, defaultImageModel }: Props) {
  ...
  return (
    <div>
      <label htmlFor="album-select" style={fieldLabel}>Album</label>
      <select
        id="album-select"
        aria-label="Album"
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
        <label htmlFor="album-prompt" style={{ ...fieldLabel, marginBottom: 0 }}>Prompt</label>
        {chatEnabled && (
          <Button variant="ghost" small busy={suggesting} disabled={!album} onClick={onSuggest}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", marginBottom: "var(--space-1)" }}>
        ...
        <span style={t.label}>Square 1:1 · applies to every song of the album.</span>
      </div>
      ...
      {image && !busy && (
        <div style={{ marginTop: "1rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", marginTop: "var(--space-3)", flexWrap: "wrap" }}>
            <Button variant={saved ? "secondary" : "primary"} small disabled={saved} onClick={apply}>

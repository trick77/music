...
describe("GenreFanartMode", () => {
  it("renders the genre picker, prompt field, and Generate action", () => {
    const html = renderToStaticMarkup(<GenreFanartMode chatEnabled={false} />);
  ...
  it("shows Suggest prompt only when chat is enabled", () => {
    const off = renderToStaticMarkup(<GenreFanartMode chatEnabled={false} />);
    expect(off).not.toContain("Suggest prompt");
    const on = renderToStaticMarkup(<GenreFanartMode chatEnabled={true} />);

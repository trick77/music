...
describe("Hero", () => {
  ...
  it("labels ranked slides #1/#2/#3 and renders a dot control per slide", () => {
    const html = renderToStaticMarkup(
      <Hero
        hero={hero}
        items={[
          item("s1", "Neon Undertow"),
          item("s2", "Chrome Sunset"),
          item("s3", "Afterimage"),
        ]}
        currentId={null}
        playing={false}
  ...
  it("keeps a generic 'Featured song' eyebrow for the unranked no-plays fallback", () => {
    const html = renderToStaticMarkup(
      <Hero
        hero={null}
        items={[item("s1", "Neon Undertow", [], false)]}
        currentId={null}
        playing={false}
        onPlay={() => {}}
        onShare={() => {}}
  ...
  it("renders each genre as a middle-dot separated link to its genre page", () => {
    const html = renderToStaticMarkup(
      <Hero
        ...
        items={[
          item("s1", "Neon Undertow", [
            { name: "Synthwave", id: "g7" },
            { name: "Dream Pop", id: "g9" },
          ]),
        ]}
        currentId={null}
        playing={false}
  ...
  it("shows an unresolved genre as plain text, not a link", () => {
    const html = renderToStaticMarkup(
      <Hero
        hero={hero}
        items={[item("s1", "Neon Undertow", [{ name: "Mystery", id: null }])]}
        currentId={null}
        playing={false}
        onPlay={() => {}}
        onShare={() => {}}
...
describe("Chapter", () => {
  ...
  it("renders the genre chapter with its songs and no AI reference", () => {
    const html = renderToStaticMarkup(
      <Chapter chapter={chapter} onPlay={() => {}} />,
    );
    expect(html).toContain("Synthwave");

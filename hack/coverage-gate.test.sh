...
#
# Both binary entrypoints are present and both are wholly uncovered, because the
# exclusion is on the whole cmd/ tree rather than a single binary. Each one moves
# the number to a materially different value if it leaks in, so this fixture
# discriminates between the correct rule and either half-applied variant:
#   both excluded (correct) 80/100 = 80.0%
#   only cmd/music excluded  80/150 = 53.3%
#   nothing excluded         80/200 = 40.0%
...
<coverage>
  <packages>
    <package name="github.com/trick77/music/internal/auth">
      <classes>
    ...
    <package name="github.com/trick77/music/cmd/music">
      <classes>
        <class name="main" filename="cmd/music/main.go">
          <lines>$(gen_lines 100 0)</lines>
        </class>
      </classes>
    </package>
    <package name="github.com/trick77/music/cmd/mockoidc">
      <classes>
        <class name="main" filename="cmd/mockoidc/main.go">
          <lines>$(gen_lines 50 0)</lines>
        </class>
      </classes>
    </package>
  </packages>
...
case "$out" in
  *80.0*) echo "  ok   excludes all of cmd/ (reports 80.0%, not 53.3% or 40.0%)" ;;
  *)      echo "  FAIL excludes all of cmd/ — got: $out"; fail=1 ;;
esac
...
<coverage>
  <packages>
    <package name="github.com/trick77/music/internal/lines">
      <classes>

...
describe("genre browse", () => {
  it("listGenres unwraps data.genres", async () => {
    const f = mockFetch({ genres: [{ id: "g1", name: "Synthwave", songCount: 4 }] });
  ...
  it("getGenre fetches the detail by id", async () => {
    const f = mockFetch({ genre: { id: "g1" }, songs: [], fanart: [], backgroundId: "", heroId: "" });
  ...
  it("getGenre surfaces the status", async () => {
    mockFetch({}, false, 404);
    await expect(getGenre("nope")).rejects.toThrow("failed to load genre (404)");
  });
});
...
describe("patchGenre", () => {
  it("PATCHes only the supplied fields", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/genres/g1", expect.objectContaining({ method: "PATCH", headers: JSON_HEADERS }));
  ...
  it("passes the background/hero/clear markers through untouched", async () => {
    const f = mockFetch({});
    await patchGenre("g1", { backgroundFanartId: "f1", heroFanartId: "f2", clearHero: "1" });
    expect(sentJson(f)).toEqual({ backgroundFanartId: "f1", heroFanartId: "f2", clearHero: "1" });
  });

  it("throws with the status", async () => {
    mockFetch({}, false, 400);
    await expect(patchGenre("g1", { name: "x" })).rejects.toThrow("save failed (400)");
  });
});
...
describe("fanart upload and generation", () => {
  ...
  it("uploadFanart throws with the status", async () => {
    mockFetch({}, false, 413);
    await expect(uploadFanart("genre", "g1", new File(["x"], "a.png"))).rejects.toThrow("fanart upload failed (413)");
  });

  it("generateFanart sends the prompt, kind, genre and model", async () => {
    const f = mockFetch({ id: "f2", status: "generating" });
    const res = await generateFanart("rain-slick street", "genre", "g1", "flux-2-pro");
    expect(f).toHaveBeenCalledWith("/api/fanart/generate", expect.objectContaining({ method: "POST", headers: JSON_HEADERS }));
    expect(sentJson(f)).toEqual({ prompt: "rain-slick street", kind: "genre", genreId: "g1", model: "flux-2-pro" });
  ...
  it("generateFanart throws with the status", async () => {
    mockFetch({}, false, 429);
    await expect(generateFanart("p", "hero", "g1")).rejects.toThrow("generate failed (429)");
  });

  it("getFanartMeta asks for the metadata variant, not the image", async () => {
    const f = mockFetch({ id: "f1", status: "ready", width: 1024, height: 1024 });
  ...
  it("getFanartMeta throws with the status", async () => {
    mockFetch({}, false, 404);
    await expect(getFanartMeta("f1")).rejects.toThrow("fanart meta failed (404)");
  });
});
...
describe("genre prompt helpers", () => {
  it("suggestGenrePrompt POSTs and unwraps the prompt", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/genres/g1/suggest-prompt", { method: "POST" });
  ...
  it("suggestGenrePrompt throws with the status", async () => {
    mockFetch({}, false, 503);
    await expect(suggestGenrePrompt("g1")).rejects.toThrow("suggest failed (503)");
  });
  ...
  it("refineGenrePrompt sends the prompt and instruction", async () => {
    ...
    expect(f).toHaveBeenCalledWith("/api/genres/g1/refine-prompt", expect.objectContaining({ method: "POST", headers: JSON_HEADERS }));
    expect(sentJson(f)).toEqual({ prompt: "chrome and neon", instruction: "at night" });
  ...
  it("refineGenrePrompt throws with the status", async () => {
    mockFetch({}, false, 500);
    await expect(refineGenrePrompt("g1", "a", "b")).rejects.toThrow("refine failed (500)");
  });
});

...
// Typeahead suggestion sits on the active surface with UI-scale text (design-system.md).
...
export function PlaylistEditor({ existing, onClose, onSaved }: Props) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 70, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(2px)", WebkitBackdropFilter: "blur(2px)", display: "grid", placeItems: "center", padding: "1rem" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 560, maxWidth: "100%", maxHeight: "88vh", overflowY: "auto", background: "var(--color-panel)", border: "1px solid var(--color-border)", borderRadius: 14, padding: "1.25rem" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <label style={{ ...fieldLabel, marginTop: "0.6rem" }}>Description · optional</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} className={controlClass} />
          </div>
        </div>

        <div style={{ ...t.label, marginBottom: 4 }}>Songs · {songs.length}</div>
        {songs.map((song, i) => (
        ...
        <input placeholder="Add songs — search by title or artist…" value={query}
          ...
          className={controlClass} style={{ marginTop: "0.75rem" }} />
        {matches.map((song) => (
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.6rem", marginTop: "1.25rem" }}>
          ...
          <Button busy={busy} disabled={!name} onClick={save}>

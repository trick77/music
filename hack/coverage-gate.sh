...
# cmd/ (main() wiring: cmd/music and the cmd/mockoidc dev helper) is deliberately
# not counted. Unit tests cannot reach binary entrypoints — flag parsing, service
# wiring, signal handling — so counting them would add a constant block of
# permanently uncovered lines that no reachable test could ever move.
set -euo pipefail

  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem icon="play" onClick={p.onPlayNext}>
          Play next
        </MenuItem>
        <MenuItem icon="openItems" onClick={p.onAddToQueue}>
          Add to queue
        </MenuItem>
        {/* Playlist-building is signed-in only (spec §1); omit for anonymous. */}
        {p.authenticated && (
          <MenuItem icon="plus" onClick={p.onAddToPlaylist}>
            Add to playlist…
          </MenuItem>
        )}

        {p.authenticated && <MenuSeparator />}
        <MenuItem icon="download" href={`/api/songs/${p.song.id}/download`}>
          Download
        </MenuItem>
        ...
        {p.authenticated && !!p.song.coverArtId && (
          <MenuItem
            icon="imageDown"
            href={`/api/songs/${p.song.id}/cover/download`}
          >
            Download cover art
          </MenuItem>
        )}
        {/* An unpublished song is hidden from anonymous listeners, so its shared
            ...
            published. */}
        {p.song.published && (
          <MenuItem icon="share" onClick={p.onShare}>
            Share
          </MenuItem>
        )}
        ...
        {p.authenticated && (
          <>
            <MenuSeparator />
            <MenuItem icon="edit" onClick={p.onEdit}>
              Edit…
            </MenuItem>
            ...
            <MenuItem icon="globe" onClick={p.onPublish}>
              {p.song.published ? "Unpublish" : "Publish"}
            </MenuItem>

            <MenuSeparator />
            <MenuItem icon="trash" danger onClick={p.onDelete}>
              Delete song
            </MenuItem>
          </>
        )}

...
describe("UnpublishedBadge", () => {
  it("when the song is unpublished, then each placement is tagged for its breakpoint", () => {
    expect(
      renderToStaticMarkup(<UnpublishedBadge show placement="actions" />),
    ).toContain('class="row-badge-actions"');
    expect(
      renderToStaticMarkup(<UnpublishedBadge show placement="meta" />),
    ).toContain('class="row-badge-meta"');
  });
  ...
  it("when the song is published (or the viewer is anonymous), then nothing renders", () => {
    ...
    expect(
      renderToStaticMarkup(
        <UnpublishedBadge show={false} placement="actions" />,
      ),
    ).toBe("");
    expect(
      renderToStaticMarkup(<UnpublishedBadge show={false} placement="meta" />),
    ).toBe("");
  });
  ...
  it("when it renders, then the label is spelled out, not truncated", () => {
    ...
    const html = renderToStaticMarkup(
      <UnpublishedBadge show placement="meta" />,
    );

    ...
    >
      ...
      {nowPlaying && (
        <>
          ...
          <div style={{ padding: "0.4rem 0", marginBottom: "0.5rem" }}>
            <strong style={t.ui}>{nowPlaying.title}</strong>
            <div style={t.label}>{nowPlaying.artistName}</div>
          </div>
        </>
      )}
      ...
      {queue.map((song, i) => (
        <div
          ...
          onDrop={() => {
            if (drag !== null) onChange(reorder(queue, drag, i));
            setDrag(null);
          }}
          style={{
            display: "flex",
        ...
        >
          ...
          <button
            onClick={() => onChange(removeAt(queue, i))}
            style={{
              background: "none",
              border: "none",
              color: "var(--color-muted)",

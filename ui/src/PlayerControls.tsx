...
export function Divider({ color = "var(--color-border)" }: { color?: string }) {
  return (
    <span
      aria-hidden
      data-divider
      style={{ width: 1, height: 24, background: color, flexShrink: 0 }}
    />
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "none",
        cursor: "pointer",
        color: on ? "var(--color-accent-strong)" : "var(--color-muted)",
      }}
    >
...
export function AirplayButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      style={
        active ? { ...iconBtn, color: "var(--color-accent-strong)" } : iconBtn
      }
    >
...
export function ImmersiveControls({
  ...
  return (
    <div
      style={{
        position: "absolute",
        bottom: IMMERSIVE_CONTROLS_BOTTOM,
        left: 0,
        right: 0,
        display: "flex",
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
      <button
        aria-label="Previous"
        onClick={onPrev}
        style={{ ...iconBtn, color: "var(--color-ink)" }}
      >
        <Glyph name="prev" size={size - 2} />
      </button>
      <button
      ...
      <button
        ...
        style={
          canNext
            ? { ...iconBtn, color: "var(--color-ink)" }
            : { ...iconBtn, cursor: "default", opacity: 0.45 }
        }
      >
